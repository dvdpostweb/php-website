...
$current_page_name = 'gift_form2.php';
...
if (!tep_session_is_registered('customer_id')) {
	if (strlen($HTTP_COOKIE_VARS['email_address']) > 0){
		...
		if ($customers['customers_id']>0){
	        ...
	        $date_now = date('Ymd');	        
	        tep_db_query("update customers set customers_info_date_of_last_logon = now(), customers_info_number_of_logons = customers_info_number_of_logons+1 where customers_id = '" . $customer_id . "'");	        
		}else{
