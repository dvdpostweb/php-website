  ...
  $conditions = $dom_values['text'];
	...
	/*switch ($code['activation_group']){
	    case 39: //spector
			...
			$email_text = str_replace('���abo_type���',  $product_info_values['products_name'] , $email_text);
			$email_text = str_replace('���login���',  $customers['customers_email_address'] , $email_text);
			$email_text = str_replace('���customers_address���',  $customers_addr['entry_street_address'] . ' ' . $customers_addr['entry_postcode'] . ' ' . $customers_addr['entry_city'], $email_text);
...
function mail_message($customer_id, $mail_id, $data, $site = 'dvdpost')
{
  ...
  else
  {
	...
	if($mail_id == 645)
	{
      ...
      #tep_mail('gs@dvdpost.be', 'gs@dvdpost.be', 'mail 645', $email_text , 'dvdpost@dvdpost.be', 'dvdpost@dvdpost.be');
		}
	}
	$data['display']='block';
