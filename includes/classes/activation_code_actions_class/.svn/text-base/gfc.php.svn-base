	...
	function execute($son_id)
	{
		...
		if(!empty($son_id))
		{
			if($this->data['points']>=200)
			{
					...
					$father = tep_db_query("select * from customers where customers_id = '" . $this->data['customers_id'] . "' ");
					...
					$sql="insert into mem_get_mem_used (date, father_id, father_abo_type, son_id , son_abo_type , points,expected_points ) values (now(), " . $this->data['customers_id']  . ", ".$father_value['customers_abo_type'].", " . $son_id . ", ".$father_value['customers_abo_type'].", '0',".$this->data['points']." )";
					//echo $sql.'<br />';
					$data = array();
