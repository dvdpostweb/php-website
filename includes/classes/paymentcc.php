...
// class constructor
    function payment() {
      ...
      if (MODULE_PAYMENT_INSTALLED) { 
	    $this->modules = explode(';', PAYMENT_METHOD); // get array of installed/active modules

        reset($this->modules);
        while (list(, $value) = each($this->modules)) { // get module defines
          include(DIR_WS_INCLUDES . 'payment/' . $language . '/' . $value);
          include(DIR_WS_INCLUDES . 'payment/' . $value);
    ...
    function pre_confirmation_check() {
      ...
      if (MODULE_PAYMENT_INSTALLED) {
        ...
        if (!$payment_module_selected) {
          tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENTCC, 'error_message=' . urlencode(ERROR_NO_PAYMENT_MODULE_SELECTED), 'SSL'));
        }
      }
    }
     ...
     function getpaymeth() {
      ...
      if (MODULE_PAYMENT_INSTALLED) {
        ...
        while (list(, $value) = each($this->modules)) {
          $class = substr($value, 0, strrpos($value, '.'));
          if ( ($GLOBALS[$class]->code == $payment) && ($GLOBALS[$class]->enabled) ) {
            $confirmation_string .= $GLOBALS[$class]->code ;
          }
        }
      }
      return $confirmation_string;
