...
Class Cookie {
    ...
    public function getCookie($cookieName=null, $decrypt=false)
    {
        ...
        if(isset($_COOKIE[$cookieName])){
            return ($decrypt?$this->cookieEncryption($_COOKIE[$cookieName],true):$_COOKIE[$cookieName]);
        } else {
    ...
    //echo '1. '.$this->cookieName.'<br />2. '.$this->cookieData.' <br />3.  '.$this->cookieExpire.'  <br />4. '.$this->cookiePath.'  <br /> 5. '.$this->cookieDomain.'  <br />6. '.$this->cookieSecure." <br /> 7. ".$this->cookieHTTPOnly.'<br />'.$ret.'<br />';
    ...
    public function setValue($value=null, $encrypt=false)
    {
        if(!is_null($value)){
            ...
            $data = ($encrypt?$this->cookieEncryption($value):$value);
...
//echo 'domaine-->'.$this->cookieDomain;
        }
        return $this;
    }
    ...
    private function cookieEncryption($str=null, $decrypt=false)
    {
        ...
        if($decrypt){
            $return = mcrypt_decrypt(MCRYPT_3DES, $key, $str, MCRYPT_MODE_ECB, $iv);
        } else {
