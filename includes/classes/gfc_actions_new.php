...
class gfc_actions {
	...
	public function execute($customers_id ,$codes)
	{
		...
		{	
			...
			{
				$sql='select * from activation_actions where activation_code_id='.$row['activation_id'];
				$query=tep_db_query($sql);
				if(!tep_db_fetch_array($query))
				{

					$sql='insert into activation_actions (activation_code_id ,class_id, customer_id) values('.$row['activation_id'].',1,'.$customers_id.')';
