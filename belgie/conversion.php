<?php
	...
	// Set track-back URL.
	$trackBackURL = ($useHttps ? 'https' : 'http') . '://' . ($conversionType === 'lead' ? 'tl' : 'ts') . '.tradetracker.net/?cid=' . $campaignID . '&pid=' . $productID . '&data=' . urlencode($trackingData) . '&type=' . $trackingType . '&tid=' . urlencode($transactionID) . '&tam=' . urlencode($transactionAmount) . '&eml=' . urlencode($email) . '&descrMerchant=' . urlencode($descriptionMerchant) . '&descrAffiliate=' . urlencode($descriptionAffiliate);
