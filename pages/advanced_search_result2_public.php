...
<table width="93%" border="0" align="center" cellpadding="0" cellspacing="0">
 <tr>
	<td  width="170" valign="top" id="left_menu" class="limitedathome">
    	...
    	<td align="center" class="dvdpost_formula_grey" valign="top">
		<?php  if (strlen($HTTP_GET_VARS['keywords'])>1){?>
			<table width="560" border="0" align="center" cellpadding="0" cellspacing="0">
			  <tr>
				<td height="24"  class="actor_director_count_text_v4">
					...
					<h2>
					<?php  echo $count_movie[count].' <a href="advanced_search_result2_public.php?type=movie&keywords='.htmlspecialchars($keywords).'">'.TEXT_BY_TITLE.'</a>';?> |
					<?php  echo $count_series[count].' <a href="advanced_search_result2_public.php?type=serie&keywords='.htmlspecialchars($keywords).'">'.TEXT_SERIE.'</a>';?> |
					<?php  echo $directors_nb.' <a href="advanced_search_result2_public.php?type=directors&keywords='.htmlspecialchars($keywords).'">'.TEXT_DIRECTORS.'</a>';?> |
					<?php  echo $actors_nb.' <a href="advanced_search_result2_public.php?type=actors&keywords='.htmlspecialchars($keywords).'">'.TEXT_ACTORZ.'</a>';?> |
			...
			<table width="560" border="0" align="center" cellpadding="0" cellspacing="0">
		    <?php  
				//BEN001 $select_str_products = "select if(to_days(p.products_date_added)=to_days(curdate()),1,0) added_today, p.products_id, pd.products_name, p.products_availability, p.products_language_fr, p.products_undertitle_nl, p.products_next, p.products_date_available,p.products_rating ,p.products_directors_id,pd.products_description,pd.products_image_big from " . TABLE_PRODUCTS . " p left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id where p.products_id > 9 and pd.products_name like '%" . $HTTP_GET_VARS['keywords'] . "%' group by p.products_id, pd.products_name order by pd.products_name";
			//$select_str_products = "select if(to_days(p.products_date_added)=to_days(curdate()),1,0) added_today, p.products_id, p.products_media, pd.products_name, p.products_availability, p.products_language_fr, p.products_undertitle_nl, p.products_next, p.products_date_available,p.products_rating ,p.products_directors_id,pd.products_description,pd.products_image_big from " . TABLE_PRODUCTS . " p left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id where p.products_type = 'DVD_NORM' and p.products_status !=-1 and pd.products_name like '%" . $HTTP_GET_VARS['keywords'] . "%' group by p.products_id, pd.products_name order by pd.products_name"; //BEN001
			if($_GET['type']=='serie')
			{
			...
			else
			{
				...
				$select_str_products = "select if(to_days(p.products_date_added)=to_days(curdate()),1,0) added_today, p.products_id, p.products_media, pd.products_name, p.products_availability, p.products_language_fr, p.products_undertitle_nl, p.products_next, p.products_date_available,p.products_rating ,p.products_directors_id,pd.products_description,pd.products_image_big ";
			...
			?>
 				<td><a href='<?= actors_link($lang_short,$row['actors_name'],$row['actors_id']); ?>?showtitles=1'> &bull; <?= truncate($row['actors_name'],26,'...' )?></a></td>
			<?php 
				if($i % 3==3){echo '</tr>';}
			$i++; 	
			}
			...
			?>
			<td><a href='<?= directors_link($lang_short,$row['directors_name'],$row['directors_id']); ?>'> &bull; <?= truncate($row['directors_name'],26,'...' )?></a></td>
			<?php 
				if($i % 3==3){echo '</tr>';}
				$i++; 	
			}
		                          ...
		                          <?php  
					              	switch ($tab[$cpt_list]['products_media']){
										case 'BlueRay' :
											...
											echo '<img class="bluborder" src="'.$constants['DIR_DVD_WS_IMAGES'].$tab[$cpt_list]['products_image_big'].'" border="0" width="108" height="144" alt="'.$tag_dvd.'" valign="top"></a></td></tr></table>';
											echo '<td height="20" align="center" valign="middle" bordercolor="#FFFFFF" class="DVD_TITLE_BLU ">';
											break;
										
										default :
											echo '<a href="'. products_link($lang_short,$tab[$cpt_list]['products_name'],$tab[$cpt_list]['products_id']).'">';
											echo '<img src="'.$constants['DIR_DVD_WS_IMAGES'].$tab[$cpt_list]['products_image_big'].'" border="0" width="108" height="162" alt="'.$tag_dvd.'" class="border_dvd">';
											echo '</a></td>';
		        ...
		        <td width="120" valign="top"><table width="100%"  border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr >
		                    ...
		                    <td height="20" bordercolor="#FFFFFF" bgcolor="#ffffff" class="SLOGAN_DETAIL"><?php  
															...
															echo '<b class="TYPO_STD_BLACK_bold"><a href="'. directors_link($lang_short,$directors_values['Directors_name'],$tab[$cpt_list]['products_directors_id']). '" class="basiclink"><u>'. $directors_values['Directors_name']. '</u></a><img src="'. DIR_WS_IMAGES . 'blank.gif" width="3" height="3"></b>';						 
															?>
		                    </td>
                            </tr>
                            <tr>
