  ...
  <form name="form1" action="mywishlistupdate.php" method="post">
    	  ...
    	  <tr>
			<td colspan="4">
		 		<table width="736">
		 		<?php       			
				$wl_query = tep_db_query("select distinct if(wa.date_assigned is null,0,1) already_rented, pd.products_name, p.products_next, p.products_availability, w.wl_id, w.rank, w.product_id, date_format(p.products_date_available,'%Y-%m-%d') day from " . TABLE_WISHLIST . " w inner join " . TABLE_PRODUCTS . " p on p.products_id=w.product_id left join " . TABLE_WISHLIST_ASSIGNED . " wa on wa.customers_id=w.customers_id and wa.products_id=w.product_id left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on pd.products_id = w.product_id and pd.language_id='" . $languages_id . "' where w.customers_id = '" . $customer_id . "' and p.products_availability >= 0 and w.wishlist_type = 'DVD_NORM' order by w.rank"); //BEN001
...
//								//	echo '<a href="' . FILENAME_PRODUCT_REVIEWS_WRITE . '?cPath=21&products_id='. $wl['product_id'] .'">' . tep_image_button('button_write_review.gif', IMAGE_BUTTON_WRITE_REVIEW) . '</a>';						
  ...
  <?php  
	...
	$wl_query = tep_db_query("select if(wa.date_assigned is null,0,1) already_rented, pd.products_name, p.products_next, p.products_availability, w.wl_id, w.rank, w.product_id, date_format(p.products_date_available,'%Y-%m-%d') day from " . TABLE_WISHLIST . " w inner join " . TABLE_PRODUCTS . " p on p.products_id=w.product_id left join " . TABLE_WISHLIST_ASSIGNED . " wa on wa.customers_id=w.customers_id and wa.products_id=w.product_id left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on pd.products_id = w.product_id and pd.language_id='" . $languages_id . "' where w.customers_id = '" . $customer_id . "' and p.products_availability =-1 and w.wishlist_type = 'DVD_NORM' order by w.rank"); //BEN001
	...
	<table width="764" border="0" align="center" cellpadding="0" cellspacing="0">
		...
		<tr>
			<td colspan="4">
				<table width="736">
				<?php 					
					$wl_query = tep_db_query("select if(wa.date_assigned is null,0,1) already_rented, pd.products_name, p.products_next, p.products_availability, w.wl_id, w.rank, w.product_id, date_format(p.products_date_available,'%Y-%m-%d') day from " . TABLE_WISHLIST . " w LEFT JOIN  " . TABLE_PRODUCTS . " p on w.product_id=p.products_id left join " . TABLE_WISHLIST_ASSIGNED . " wa on wa.customers_id=w.customers_id and wa.products_id=w.product_id left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on pd.products_id = w.product_id and pd.language_id='" . $languages_id . "' where w.customers_id = '" . $customer_id . "' and p.products_availability =-1 and w.wishlist_type = 'DVD_NORM' order by w.rank"); //BEN001
					while ($wl = tep_db_fetch_array($wl_query)) {      
					?>
					<tr align="center">
						<td width="100">
