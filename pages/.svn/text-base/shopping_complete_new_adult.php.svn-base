...
<table width="<?php  echo SITE_WIDTH_PUBLIC; ?>" align="center" border="0" cellspacing="0" cellpadding="0">
	...
	<td colspan="2" align="center">
		<table width="269" border="0" align="center" cellpadding="0" cellspacing="0">
			...
			<tr>
				...
				$dvdsale_query = tep_db_query("select * from shopping_orders where customers_id = '" . $customer_id . "' and  status=1 and products_type='DVD_ADULT'");
				...
				///count
				$countdvd_query = tep_db_query("select sum(quantity) as nbrdvd from shopping_orders where customers_id = '" . $customer_id . "' and  status=1 and products_type='DVD_ADULT'");
				$countdvd = tep_db_fetch_array($countdvd_query);
				
				
				$sc_query = tep_db_query("select * from shopping_orders so left join products p on so.products_id = p.products_id left join products_description pd on pd.products_id = so.products_id and pd.language_id='" . $languages_id . "' where p.products_type = 'DVD_ADULT' and so.customers_id = '" . $customer_id . "' and  so.status= 1 "); //BEN001
		      	while ($sc = tep_db_fetch_array($sc_query)) {
					...
					echo '<td width="40" align="center" class="slogan_detail">' . $sc['products_new_sale_price'] . '</td>';	
				    $add.='<td width="40" align="center" class="slogan_detail">' . $sc['products_new_sale_price'] . '</td>';	
					echo '<td width="25" class="slogan_red">' . $sc['quantity'] * $sc['products_new_sale_price'] . '</td>';	
				    $add.='<td width="25" class="slogan_red">' . $sc['quantity'] * $sc['products_new_sale_price'] . '</td>';	
					echo '</tr>';	
			      	$add.='</tr>';
					$dectotprice = $dectotprice + ($sc['quantity'] * $sc['products_new_sale_price']);
				}
				...
				$add.='<td width="25" class="slogan_red">' . $shipping . '</td></tr>';								
  ...
  <tr>
    <td><a class="red_slogan" href="dvdforsale_listing_new_adult.php"><u><?php  echo TEXT_BUY_CATALOG ;?></u></a> </td>
