...
<table width="677" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr> 
    <td height="142" class="limitedathome" valign="middle"> 
      	<?php 
		$customers = tep_db_query("select DATE_FORMAT( customers_abo_validityto,  '%e/%m/%Y '  ) as date_x, customers_abo_type, customers_next_abo_type from " . TABLE_CUSTOMERS. " p where p.customers_id= '".$customer_id."'");
		...
		?>
		<?php
			$sql_free="SELECT *  FROM `abo` WHERE `customerid` = ".$customer_id." order by abo_id desc";
			$abo_free = tep_db_query($sql_free);
			$abo_free_values = tep_db_fetch_array($abo_free);
			if($abo_free_values['comment']=='FREE UPGRADE')
			{	
			echo '<br><br>';
			echo TEXT_FREE_UPGRADE ;
			}
			?>
		<br><br>
