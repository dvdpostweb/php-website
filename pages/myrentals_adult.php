  ...
  <tr>
    <TD colspan="4">
		...
		include('includes/functions/isAdult.php'); //BEN001
 	 	$history_query_raw = "select o.orders_id, o.date_purchased, s.orders_status_name, IF(DATE_ADD(o.date_purchased, INTERVAL 1 MONTH) > curdate(),1,0) as ced from " . TABLE_ORDERS . " o left join " . TABLE_ORDERS_STATUS . " s on (o.orders_status = s.orders_status_id and s.language_id = '" . $languages_id . "') where o.customers_id = '" . $customer_id . "' and o.orders_status<> 6 order by orders_id DESC";
    	...
    	while ($history = tep_db_fetch_array($history_query)) {
      	...
      	<td width="132" align="center" class="TYPO_STD_BLACK">
      		<?php  
      			...
      			if ($history['orders_status'] == 12 || $history['orders_status'] == 17) {
      	...
      	while ($products_name = tep_db_fetch_array($products_name_query)) {
			if ($products_name['products_id'] < 10 ){
				...
				echo '<a class="basiclink" href="product_info_adult.php?products_id=' . $products_name['products_id'] . '">' . $products_name['products_name'] . '</a><br>';        
				}
			}
      }      
      ?>
