...
//echo $listing_sql;
  //if ($listing_numrows = 0) {  
	...
	$split=$cPath;
	$search_title= TEXT_BEST_BUY;
	$number_of_products = '0';
	$listing_sql='SELECT so.products_id, sum( so.quantity ) , pd.products_name, pd.products_image_big, p.products_sale_price, p.products_language_fr, p.products_undertitle_nl ';
	...
	if ($x!=0 && $x<25){
		$listing_sql.=' GROUP BY so.products_id ORDER  BY  `sum( so.quantity )`  DESC LIMIT '.$x.','.$limit ;}
	else{
		$x=0;
		$listing_sql.=' GROUP BY so.products_id ORDER  BY  `sum( so.quantity )`  DESC LIMIT '.$x.','.$limit ;}
