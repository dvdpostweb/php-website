...
<?php  
//RALPH-002 $actor_info = tep_db_query("select Actors_Name, Actors_image , Actors_dateofbirth , Actors_awards, Actors_description from actors_adult where Actors_id = '" . $HTTP_GET_VARS['actors_id'] . "' ");
$actor_info = tep_db_query("select Actors_Name, Actors_image , Actors_dateofbirth , Actors_awards, Actors_description from actors where actors_type = 'DVD_ADULT' and Actors_id = '" . $HTTP_GET_VARS['actors_id'] . "' "); //RALPH-002
...
<?php  
	$i=1;
	//RALPH-002 $a = tep_db_query("select products_id  from products_to_actors_adult where actors_id = '" . $HTTP_GET_VARS['actors_id'] . "'  ");
	$a = tep_db_query("select products_id  from products_to_actors where actors_id = '" . $HTTP_GET_VARS['actors_id'] . "'  "); //RALPH-002
	while ($a_v = tep_db_fetch_array($a)) {
	//RALPH-005 $product_info = tep_db_query("select p.products_id, pd.products_name, pd.products_description, p.products_model, p.products_quantity, p.products_image, pd.products_url, p.products_price, p.products_tax_class_id, p.products_date_added, p.products_date_available, p.manufacturers_id, p.products_public, p.products_runtime, p.products_year, p.products_picture_format , p.products_rating ,p.manufacturers_id, pc.categories_id, p.products_regional_code, p.products_studio, p.products_availability,p.products_language_fr, p.products_undertitle_nl, pd.products_image_big  from products_adult p, products_description_adult pd, products_to_categories_adult pc where p.products_id = '" . $a_v['products_id'] . "' and pd.products_id = '" . $a_v['products_id'] . "'  and pd.language_id = '" . $languages_id . "' and pc.products_id= '" . $a_v['products_id'] . "'");
	$product_info = tep_db_query("select p.products_id, pd.products_name,p.rating_users,p.rating_count, pd.products_description, p.products_model, p.products_quantity, p.products_image,p.products_status, pd.products_url, p.products_price,p.products_tax_class_id, p.products_date_added, p.products_date_available, p.products_studio, p.products_public, p.products_runtime, p.products_year, p.products_picture_format , p.products_rating ,	pc.categories_id,products_media, p.products_regional_code, p.products_studio, p.products_availability,p.products_language_fr, p.products_undertitle_nl, pd.products_image_big  from products p, products_description pd, products_to_categories pc where p.products_id = '" . $a_v['products_id'] . "' and pd.products_id = '" . $a_v['products_id'] . "'  and pd.language_id = '" . $languages_id . "' and pc.products_id= '" . $a_v['products_id'] . "'"); //BEN001
	$product_info_values = tep_db_fetch_array($product_info);
	if($product_info_values['products_status']!='' && $product_info_values['products_status']!=-1){
	$studio_name = tep_db_query("select studio_name from studio where studio_id = '" . $product_info_values['products_studio'] . "'");  
            ...
            <tr> 
              <td height="30" bordercolor="#FFFFFF" bgcolor="#F8E6F4"> <table width="100%"  border="0" cellspacing="0" cellpadding="0">
                  <tr > 
                    ...
                    <td height="30" bordercolor="#FFFFFF" bgcolor="#F8E6F4" class="SLOGAN_DETAIL"><span class="TYPO_STD_BLACK_bold"><a href="studio_adult.php?studio_id=<?php   echo $product_info_values['products_studio'];?>" class="basiclink"><u><?php   echo $studio_name_values['studio_name'];?></u></a></span></td>
                  </tr>
                </table></td>
            </tr>
            <tr> 
        ...
        <td width="438" valign="top"> 
          <table width="100%"  border="0" cellpadding="0" cellspacing="0">
            <tr valign="middle"> 
		  	...
		  	$categories = tep_db_query("select cd.categories_name from products_to_categories c, categories_description cd where cd.categories_id=c.categories_id and cd.language_id = '" . $languages_id . "' and c.products_id='" . $product_info_values['products_id'] . "'");
