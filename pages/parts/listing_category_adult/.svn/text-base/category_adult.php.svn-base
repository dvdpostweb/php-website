<?php 
//RALPH-005 $categories_products_query = tep_db_query("select count(*) as total from products_to_categories_adult where categories_id = '" . $current_category_id . "'");
$categories_products_query = tep_db_query("select count(*) as total from products_to_categories where categories_id = '" . $current_category_id . "'"); //RALPH-005
...
if ($cateqories_products['total'] > 0) {
		...
		// We show them all
			...
			$filterlist_sql = 'select distinct c.categories_id as id, cd.categories_name as name from ' . TABLE_PRODUCTS . ' p, ' . TABLE_PRODUCTS_TO_CATEGORIES . ' p2c, ' . TABLE_CATEGORIES . ' c, ' . TABLE_CATEGORIES_DESCRIPTION . ' cd where p.products_status = \'1\' and p.products_id = p2c.products_id and p2c.categories_id = c.categories_id and p2c.categories_id = cd.categories_id and cd.language_id = \'' . $languages_id . '\' and p.manufacturers_id = \'' . $HTTP_GET_VARS['manufacturers_id'] . '\' and products_type = "DVD_ADULT" and c.product_type="Movie" order by cd.categories_name '; //BEN001
		} else {
		...
		<tr>
			...
			//RALPH-005 $categories_descriptions = tep_db_query("select c.categories_name from categories_description_adult c where c.categories_id = '" . $current_category_id . "' and language_id = '" . $languages_id . "'");  
			$categories_descriptions = tep_db_query("select c.categories_name from categories_description c where c.categories_id = '" . $current_category_id . "' and language_id = '" . $languages_id . "'");  
			$categories_descriptions_values = tep_db_fetch_array($categories_descriptions);
			//RALPH-005 $show_parent_descriptions = tep_db_query("select c.parent_id , cd.categories_name from categories_adult  c, categories_description_adult  cd WHERE c.categories_id ='". $current_category_id ."' and c.parent_id=cd.categories_id AND c.parent_id > 0 and language_id = '" . $languages_id . "'");  
			$show_parent_descriptions = tep_db_query("select c.parent_id , cd.categories_name from categories  c, categories_description  cd WHERE c.categories_id ='". $current_category_id ."' and c.parent_id=cd.categories_id AND c.parent_id > 0 and language_id = '" . $languages_id . "'");  //RALPH-005
