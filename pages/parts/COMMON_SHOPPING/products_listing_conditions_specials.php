...
switch ($dvdlist) {
  ...
  case 'DVDPOSTCHOICE':
    ...
    break;
  case 'NEW':
    $listing_sql = $listing_sql . ' and p.products_date_added<=curdate() and p.products_date_added > DATE_SUB(now(), INTERVAL 1 MONTH) and p.products_date_available >= DATE_SUB(now(), INTERVAL 1 MONTH) and p.products_next=0 ';
    break;
  case 'LASTADDED':
