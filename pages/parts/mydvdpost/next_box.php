...
<table width="243" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
	 <td colspan="2" height="30" valign="top" align="left"><b><?php   echo TEXT_NEXT;?></b> <a href="listing_category.php?list=next" class="link_SLOGAN_MENU2">(<?php   echo TEXT_SEE_ALL ;?>)</a></td>
  	 <td width="10" background="<?php   echo  DIR_WS_IMAGES ;?>vert_line.gif"><img src="<?php   echo DIR_WS_IMAGES;?>blank.gif"></td>
	</tr>
    <?php  
		$next_sql = 'select p.products_id,  pd.products_name,pd.products_description, pd.products_image_big, p.imdb_id,p.rating_users,p.rating_count,date_format(products_date_available,"%d/%m/%Y") as date_available_formated from ' . TABLE_PRODUCTS . ' p ';
		$next_sql .= ' left join ' . TABLE_PRODUCTS_DESCRIPTION . ' pd on p.products_id = pd.products_id and pd.language_id=' . $languages_id ;
		$next_sql.= ' where p.products_next=1 and in_cinema_now = 0 and p.products_status != -1 and (p.rating_users/p.rating_count) >= 3 ';
		$next_sql.= ' and p.products_type = "DVD_NORM"'; //BEN001
		//$listing_sql:= ' and in_the_bags_next = 1 ';
		switch ($languages_id){
				case 1:
					$next_sql.= ' and p.products_language_fr >0 ';
				break;
				case 2:
					$next_sql.= ' and p.products_undertitle_nl >0 ';
				break;
				case 3:
				break;
				}
		...
		$top_tab = tep_db_cache_asso($next_sql,'products_id',14400);
