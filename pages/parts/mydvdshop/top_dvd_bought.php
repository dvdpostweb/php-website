<table width="375" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td width="149" background="<?php  echo DIR_WS_IMAGES;?>shop/titlebar/box/title_mid.gif" class="dvdpost_left_menu_title" align="left"><b><?php  echo TEXT_BEST_BUY;?></b></td>
  ...
  <tr>
    ...
    <table width="100%">
    		<tr>    			
			    <?php 
				$listing_sql='SELECT so.products_id, sum( so.quantity ) , pd.products_name, pd.products_image_big, p.products_sale_price ';
				$listing_sql.='FROM shopping_orders so ,products p, products_description pd '; 
				$listing_sql.='WHERE TO_DAYS(NOW()) - TO_DAYS(so.date) <= 15 AND so.products_id=p.products_id AND so.products_id=pd.products_id and p.quantity_to_sale > 0 and pd.language_id=' . $languages_id ;
				switch ($languages_id){
					case 1:
				...
				$listing_sql.=' GROUP  BY so.products_id ORDER  BY  `sum( so.quantity )`  DESC LIMIT 7';
				$listing = tep_db_query($listing_sql);
				$i=1;
				while ($product_info_values = tep_db_fetch_array($listing)) {
					if ($i<4){
						echo '<td width="33%" align="center"><table><tr><form name="form1" method="post" action="addtoshoppingcart.php">';
						echo '<input name="products_id" type="hidden" value="'.$product_info_values['products_id'].'">';
						echo '<td width="100" height="150" valign="top" colspan="2"><a href="product_info_shop.php?products_id='.$product_info_values['products_id'] .'"><img src="'.$constants['DIR_DVD_WS_IMAGES'].'/'.$product_info_values['products_image_big'].'" border="0" width="100" height="150" alt="'.$product_info_values['products_name'].'"></a></td>';
		...
		<br>
		<div align="right"><a href="shop_best_buy.php" class="red_slogan"><?php  echo TEXT_SEE_ALL ;?></a></div>
    </td>
