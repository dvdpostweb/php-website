  ...
  <tr>
    ...
    <td colspan="3">
		...
		<table width="100%">
    		<tr>    			
			    <?php 
				$listing_sql='SELECT  DISTINCT so.products_id, so.date, pd.products_name, pd.products_image_big, p.products_sale_price ';
				$listing_sql.=' FROM shopping_orders so, products p, products_description pd ';
				$listing_sql.='WHERE so.products_id = pd.products_id AND p.products_id = pd.products_id and p.quantity_to_sale > 0 and pd.language_id=' . $languages_id ;
				switch ($languages_id){
					case 1:
				...
				$listing_sql.=' ORDER  BY  `date`  DESC  LIMIT 5 ';
				...
				while ($product_info_values = tep_db_fetch_array($listing)) {
					if ($i<3){
						...
						echo '<td align="right"><input name="imageField" type="image" src="'.DIR_WS_IMAGES.'shop/titlebar/box/add.gif" width="27" height="21" border="0"></td></form></tr></table></td>';
							if ($i==2){echo '</tr></table><table width="100%"><tr>';}
						}else{			
							
					echo '<td width="20%" align="center"><table><tr><form name="form1" method="post" action="">';
