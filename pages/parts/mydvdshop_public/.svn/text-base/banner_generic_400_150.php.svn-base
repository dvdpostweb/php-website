 ...
 <?php  
		...
		$count_action_shop_values = tep_db_fetch_array($count_action_shop_query);
 		...
 		if ($count_action_shop_values['cpt'] >0){
			...
			while ($banner_values = tep_db_fetch_array($banner_query)){    	
	    	echo '<p><a href="'.str_replace('.php','_public.php',$banner_values['banners_url']).'"><img src="'.DIR_WS_IMAGES.'languages/'.$language.'/images/shop/'.$banner_values['banners_image'].'" border="0"  alt="Shop DVDPost"></a></p><br />';
			}
	    }	
   ?>
</div>