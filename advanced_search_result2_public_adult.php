...
$current_page_name = "advanced_search_result2_public_adult.php";
...
if (strlen($HTTP_GET_VARS['keywords'])>1){
	...
	$sql .=" from " . TABLE_PRODUCTS . " p left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id and pd.language_id = ".$languages_id." where p.products_type = 'DVD_ADUlT'  AND p.products_product_type='Movie'  ".$filter." and p.products_status != -1 and pd.products_id in(".$list.")";
	...
	$actors_nb=tep_db_num_rows($query);
	
	if(	empty($_GET['type']) && $count_movie['count']==0 && $actors_nb>0)
	{
		
		tep_redirect(tep_href_link('advanced_search_result2_public_adult.php?type=actors&keywords='.$keywords, '', 'NONSSL'));
		die();
	}
	if(empty($_GET['type']) && $count_movie['count']==0 && $actors_nb==0   && $count_directors['count']>0)
	{
			tep_redirect(tep_href_link('advanced_search_result2_public_adult.php?type=directors&keywords='.$keywords, '', 'NONSSL'));
			die();
	}
}
