...
$current_page_name = FILENAME_ADVANCED_SEARCH_RESULT2_PUBLIC;
...
if (strlen($HTTP_GET_VARS['keywords'])>1){
	...
	$list_directors=search($search,'',$index_directors,array(),false);
	...
	if(	empty($_GET['type']) && $count_movie['count']==0 && $count_series['count']>0)
	{
		...
		tep_redirect(tep_href_link('advanced_search_result2_public.php?type=serie&keywords='.$keywords, '', 'NONSSL'));
		die();
	}
	if(	empty($_GET['type']) && $count_movie['count']==0 && $count_series['count']==0 && $actors_nb>0)
	{
		
		tep_redirect(tep_href_link('advanced_search_result2_public.php?type=actors&keywords='.$keywords, '', 'NONSSL'));
		die();
	}
	if(empty($_GET['type']) && $count_movie['count']==0 && $actors_nb==0 && $count_series['count']==0  && $directors_nb>0)
	{
			tep_redirect(tep_href_link('advanced_search_result2_public.php?type=directors&keywords='.$keywords, '', 'NONSSL'));
			die();
	}
}
