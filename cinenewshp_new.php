...
$tisc = tep_db_query("select * from products p left join products_description pd on p.products_id = pd.products_id where p.products_date_available > DATE_SUB(now(), INTERVAL 1 MONTH) and p.products_date_available < now() and pd.language_id = 3 order by rand() limit 20");  
