...
<table align="right" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		if(!isset($available_lang) || count($available_lang)==0){
			$available_lang=array(1,2,3);
		}
		$nb=count($available_lang);
		$add=0;
		for($i=0;$i<$nb;++$i)
		{
			if($languages_id!=$available_lang[$i])
			{
				switch($available_lang[$i]){
				case 1:			
				...
				break;
				case 3:
					echo '<a href="' . language_switcher($PHP_SELF,$QUERY_STRING, 'language=en') . '" class="yellowlink">EN</a>';
				break;
				}
				++$add;
				if($nb>=3 && $add!=($nb-1)){
